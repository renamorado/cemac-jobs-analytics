...
This note documents the reproducible crosswalk used to align the Cameroon 2024 RGE census workbook with the legacy/admin NACAM sectors used in the Cameroon elasticity analysis. The source workbook is \texttt{Data/Cameroon/Raw/CENSUS 2024 - Copy of BASE RGE 3 BANQUE MONDIALE - Copy.xlsx}, sheet \texttt{BASE}. The current implementation uses reviewable Excel crosswalks imported and merged by Stata; it does not use fuzzy matching or hidden \texttt{replace} rules for production classification.
...
\IfFileExists{../output/tables/cmr_census_crosswalk_examples.tex}{%
    \input{../output/tables/cmr_census_crosswalk_examples.tex}%
}{%
    \input{output/tables/cmr_census_crosswalk_examples.tex}%
}%
\end{tabular}
